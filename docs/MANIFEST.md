...
| 2026-09-17 | **Third CSS collision.** `.note` is the tuner's large readout, and a callout with tone `note` renders as `class="callout note"` — `.callout` won the font size but sets no family, so callout prose inherited the tuner's monospace. Renamed `.tuner-note` / `.tuner-cents`, and swept every class name shared between components: only deliberate utilities remain |
